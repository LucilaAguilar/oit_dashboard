...
bd <- readxl::read_excel("/home/lucila/Documentos/ARU/OIT - Dashboard Empleo/ECE_dashboard_v1.xlsx", sheet = "ind_trim", range = "A1:U70")
...
bd2 <- readxl::read_excel("/home/lucila/Documentos/ARU/OIT - Dashboard Empleo/ECE_dashboard_v1.xlsx", sheet = "ind_month", range = "A1:U208")
...
### __Boliva: Tasa de Participación II - 2021__

```{r}
gauge(76.08, min = 0, max = 100, symbol = '%', gaugeSectors(
...
### __Boliva: Tasa de ocupación II - 2021__

```{r}
gauge(71.95, min = 0, max = 100, symbol = '%', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)
))
```
...
### __Tasa de desocupación II - 2021__

```{r}
gauge(5.43, min = 0, max = 100, symbol = '%', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)
))
```
...
El segundo  trimestre de 2021 la tasa de participación es de __76.08%__, número que se encuentra 3.09 puntos porcentuales por encima del reportado el segundo trimestre de 2019.  
  
### __Tasa de ocupación__

La tasa de ocupación al segundo trimestre de 2021 es de __71.95%__ (6,145,039 personas), cifra mayor en 1.54 puntos porcentuales al reportado el segundo trimestre de 2019.   

### __Tasa de desocupación__

La tasa de desocupación asciende a __5.43%__ el segundo trimestre de 2021 (352,771 personas), dato que se posiciona 1.89 puntos porcentuales por encima del reportado el segundo trimestre de 2019.  
...
```{r}
valueBox("6497810", caption = "Población económicamente activa", icon="fa-user-o", color="info")
```

### Tasa de ocupación

```{r}
valueBox("6145039", caption = "Población ocupada", icon="fa-users", color="info")
```

### Disminución de ingresos laborales

```{r}
valueBox("352771", caption = "Población desocupada", icon="fa-user", color="success")
```
...
### __Horas de trabajo II - 2020__
 
```{r}
valueBox("39.30 horas", caption="Horas de trabajo II - 2021 <br> <br> <br> Fuente: Elaboración Propia con base en las ECE. Fundación ARU, 2021", icon="fa-clock-o", color="info")
```
...
El segundo trimestre de 2021 en promedio los bolivianos trabajan __39 horas__ a la semana. A diferencia del reportado el primer trimestre de 2016 con 44 horas semanales.
...
### __BOLIVIA: Ocupados y tasa de ocupación (en porcentaje), según trimestre__ {data-width=800}
...
highchart(type = "chart") %>% 
  ...
  hc_plotOptions(line = list(marker = list(lineWidth = 50))) %>% 
  hc_caption(text = "NOTA: Para los trimestres II - 2020, III - 2020, IV - 2020, I - 2021 y II - 2021 el Instituto Nacional de Estadística no publicó la información.") %>% 
...
### __BOLIVIA: Empleo Informal y Sector Informal, según trimestre__ {data-width=800}
...
### __BOLIVIA: Población económicamente activa y tasa de participación (en porcentaje) para el área urbana y rural, según trimestre__ {data-width=800}
...
highchart(type = "chart") %>% 
  ...
  hc_caption(text = "NOTA: Los datos corresponden al área urbana de Bolivia. Para el trimestre I – 2021 y II - 2021 el Instituto Nacional de Estadística no publicó la información de ocupados en servicios y otros.") %>% 
  hc_credits(text = "Fuente: Elaboración propia. Fundación ARU, 2021.",
             enabled = T) %>% 
  hc_exporting(filename = "custom-file-name", enabled = T) %>% 
  hc_scrollbar(enabled = F) %>% 
...
highchart(type = "chart") %>% 
  ...
  hc_add_theme(hc_theme_hcrt() )%>% hc_tooltip(split = T, sort = F) %>%
  hc_plotOptions(series = list(stacking = "normal")) %>% 
  hc_caption(text = "NOTA: Los datos corresponden al área urbana de Bolivia. Para el trimestre I – 2021 y II - 2021 el Instituto Nacional de Estadística no publicó la información.") %>% 
  hc_credits(text = "Fuente: Elaboración propia. Fundación ARU, 2021.",
             enabled = T) %>% 
  hc_exporting(filename = "custom-file-name", enabled = T) %>% 
  hc_scrollbar(enabled = F) %>% 
...
En general se observa que en las tres categorías ocupacionales el empleo equivalente a tiempo completo tuvo una caída a partir del segundo trimestre de 2020. Sin embargo, el empleo equivalente a tiempo completo muestra una recuperación en las tres categorías a partir del cuarto trimestre de 2020. 
...
### __BOLIVIA: Empleo Informal y Sector Informal para el área urabana y rural, según trimestre__ {data-width=800}
...
highchart(type = "chart") %>% 
  ...
  hc_add_theme(hc_theme_hcrt() )%>% hc_tooltip(split = T, sort = F) %>%
  hc_plotOptions(series = list(stacking = "normal")) %>% 
  hc_caption(text = "NOTA: Los datos corresponden al área urbana de Bolivia. Para los meses de enero, febrero, marzo, abril, mayo, junio de 2021 el Instituto Nacional de Estadística no publicó la información.") %>% 
  hc_credits(text = "Fuente: Elaboración propia. Fundación ARU, 2021.",
             enabled = T) %>% 
  hc_exporting(filename = "custom-file-name", enabled = T) %>% 
  hc_scrollbar(enabled = F) %>% 
...
Entre octubre de 2015 y enero de 2021 se observa que no hubo mucha variación en cuanto a la distribución de la población ocupada en las cuatro actividades económicas. El porcentaje de ocupados en agricultura en diciembre de 2020 es de 5.75%, cifra que es 0.65 puntos porcentuales mayor en relación al reportado en diciembre de 2019. El porcentaje de ocupados en industria es de 13.36%, cifra que es 0.2 puntos porcentuales menor al reportado en diciembre de 2019. En cuanto al porcentaje de ocupados en servicios y otros, las cifras a diciembre de 2020 son de 62.83% y 17.79% respectivamente, mostrando una variación de 0.25 y -0.7 puntos porcentuales en relación a las cifras reportadas en diciembre de 2019.
...
### __BOLIVIA: Empleo Informal y Sector Informal, según mes__ {data-width=800}
...
### __BOLIVIA: Empleo Informal y Sector Informal para el área urbana y rural, según mes__ {data-width=800}
...
highchart(type = "chart") %>% 
  ...
  hc_add_theme(hc_theme_hcrt() )%>% hc_tooltip(split = T, sort = F) %>%
  hc_plotOptions(line = list(marker = list(lineWidth = 50))) %>% 
  hc_caption(text = "La tasa de desocupación muestra la relación porcentual de la Población Desocupada (PD) respecto a la Población Económicamente Activa (PEA) que mide el desempeño del mercado laboral y la economía en su conjunto. Asimismo, mide la utilización de la mano de obra en el mercado laboral y el fracaso de encontrar un trabajo. <br> NOTA: Para los meses abril, mayo, junio, julio, agosto, septiembre, octubre, noviembre, diciembre de 2020 y enero, febrero, marzo, abril, mayo, junio de 2021 del área rural el Instituto Nacional de Estadística no publicó la información.") %>% 
  hc_credits(text = "Fuente: Elaboración propia. Fundación ARU, 2021.",
             enabled = F) %>% 
  hc_exporting(filename = "custom-file-name", enabled = T) %>% 
  hc_scrollbar(enabled = F) %>% 
...
Los ingresos laborales urbanos por la actividad principal que desempeña la población ocupada __+__ y la población remunerada __++__ presentan una tendencia positiva entre 2015 y 2020.  Sin embargo, el indicador presentan una fuerte caída en marzo de 2020.
...
highchart(type = "chart") %>% 
  ...
  hc_add_theme(hc_theme_hcrt() )%>% hc_tooltip(split = T, sort = F) %>%
  hc_plotOptions(line = list(marker = list(lineWidth = 50))) %>% 
  hc_caption(text = "NOTA: Para los meses abril, mayo, junio, julio, agosto, septiembre, octubre, noviembre,  diciembre de 2020, enero, febrero, marzo, abril, mayo, junio de 2021 del área rural y enero, febrero, marzo, abril, mayo, junio de 2021 del área urbana el Instituto Nacional de Estadística no publicó la información. <br> *La población ocupada se define como toda persona que por lo menos trabajó una hora en alguna actividad económica la semana anterior. <br> **La población remunerada son todas aquellas personas ocupadas que tienen un ingreso mayor a cero.") %>% 
  hc_credits(text = "Fuente: Elaboración propia. Fundación ARU, 2021.",
             enabled = T) %>% 
  hc_exporting(filename = "custom-file-name", enabled = T) %>% 
  hc_scrollbar(enabled = F) %>% 
